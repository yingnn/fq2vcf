...
function set_log1_start() {
    info="`get_info "$1"` start"
    echo $info >> .log1
}


function set_log1_done() {
    info="`get_info "$1"` done"
    echo $info >> .log1
    exit 0
}
...
function default_() {
    ...
    echo general multi threads $threads
    echo general java memory $java_memory
    echo
}
